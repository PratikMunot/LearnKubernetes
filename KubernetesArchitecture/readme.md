...
> - A Pod is a functional unit ie. just a thin wrapper or layer of abstraction around every container.
...
> - All the multiple containers running inside a POD share that Execution Environment of that POD. Each POD gets its own IP.
...
> Each pod can have multiple containers but usually there is only one container per pod


### Service   
> - Service is basically a static/permanent IP Address which can be attached to a set of pods that work together or single Pod as well. PODs that have same set of functions are abstracted into sets called services.
> - A same single service can be run via multiple pods.
> - Lifecycle of Pods and service are not connected that means, When a pod dies a new pod arrives with new IP. Also when we scale up the PODs, all the newly added pods are also having new and different Ips. When we scale down then we may shut the pods which clients might be using. So depending on PODs IP is very unreliable and risky. To solve this problem we have service object which sits in front of the pods and provides a stable IP and DNS name
...
> - External service – is a service which we create inorder to open services to external sources(open application in browser). Example of external service can be - http://120.59.100.5:8080
...
> - https://myapp.com    is the ingress and ingress reroutes it to http://120.59.100.5:8080 which is basically a service.

### ConfigMap  
> - Its an external configuration of the application.
> - It contains configuration data like urls of database or some other services we use.
...
### Volumes 
> - Volumes helps you to attaches physical storage on harddrive to your pod and that storage could be local hardrive or cloud storage or other VM etc.
...
> - It is similar to a container volume in Docker, but a Kubernetes volume applies to a whole pod and is mounted on all containers in the pod. Also, a pod can have multiple volumes (possibly of different types) associated.
